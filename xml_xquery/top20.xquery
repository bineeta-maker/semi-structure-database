...
<html>
    <body>
        <h1>ECS789P CourseWork - Solution#4</h1>
        <table
            border="1"
            cellpadding="10px"
            cellspacing="5px">
            <tr><th>Target</th><th>Successor</th><th>Frequency</th></tr>
            {
                ...
                (:List of all the word in the given four .xml files :)
                let $all_words := (
                for $xml in collection("./?select=*.xml")
                for $words in $xml//w
                return
                ...
                and count of unique word from the overall $all_words list and return only first 20 results :)
                for $top20 in (for $unique in distinct-values($successor_List)
                ...
                return
                    <tr><td>{distinct-values($has)}</td><td>{$unique}</td><td>{fn:round($prob, 2)}</td></tr>)[position() le 20]
                return
                    $top20   
            }
        </table>
